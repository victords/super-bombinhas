...
module AGL
	...
	class TextField
		...
		def initialize x, y, img, cursor_img = nil, text = "", margin_x = 0, margin_y = 0, max_length = 100
			@x = x
			@y = y
			@img = Res.img img
			@w = @img.width
...
#							else
		...
		def insert_char char
			if @text.length < @max_length
				...
				@nodes.insert @cur_node + 1, @nodes[@cur_node] + G.font.text_width(char)
				for i in (@cur_node + 2)..(@nodes.size - 1)
					@nodes[i] += G.font.text_width(char)
				end
				@cur_node += 1
				set_cursor_visible
			end
		...
		def remove_interval
			...
			interval_width = 0
			for i in min...max
				interval_width += G.font.text_width(@text[i])
		...
		def remove_char back
			@cur_node -= 1 if back
			char_width = G.font.text_width(@text[@cur_node])
			@text[@cur_node] = ""
		...
		def draw
			...
			if @cursor_visible
				if @cursor_img; @cursor_img.draw @text_x, @text_y, 0
					...
					G.window.draw_quad @nodes[@cur_node], @text_y, 0xff000000,
					                   @nodes[@cur_node] + 1, @text_y, 0xff000000,
					                   @nodes[@cur_node] + 1, @text_y + G.font.height, 0xff000000,
					                   @nodes[@cur_node], @text_y + G.font.height, 0xff000000, 0
				end
			end
			G.font.draw @text, @text_x, @text_y, 0, 1, 1, 0xff000000
			
			if @anchor1 and @anchor2
				G.window.draw_quad @nodes[@anchor1], @text_y, 0x80000000,
				                   @nodes[@anchor2] + 1, @text_y, 0x80000000,
				                   @nodes[@anchor2] + 1, @text_y + G.font.height, 0x80000000,
				                   @nodes[@anchor1], @text_y + G.font.height, 0x80000000, 0
			end
		end
	end
end
